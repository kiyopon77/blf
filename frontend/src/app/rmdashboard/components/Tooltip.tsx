...
export default function PlotTooltip({
  ...
  onMouseEnter: () => void
  onMouseLeave: () => void
  onStatusChange: (floorId: number, status: string) => void
}) {
  ...
  return (
    ...
    >
      <div className="bg-white border border-gray-200 rounded-xl shadow-2xl shadow-black/10 w-72 overflow-hidden">
        ...
        <div className="px-4 py-3 border-t border-gray-100">
          ...
          <div className="grid grid-cols-2 gap-2">
            {Object.entries(statusConfig).map(([key, cfg]) => {
              const apiStatus = key.toUpperCase().replace(/\s+/g, "_")

              return (
                <button
                  key={key}
                  disabled={loading || floor.status === key}
                  onClick={() => handleStatusChange(apiStatus)}
                  className={`
          text-[10px] px-2 py-1 rounded-md border transition disabled:opacity-50
          flex items-center gap-1.5 justify-center hover:cursor-pointer disabled:cursor-default
